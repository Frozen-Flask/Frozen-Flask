...
``FREEZER_RELATIVE_URLS_PRETTY``
    Only useful in combination with ``FREEZER_RELATIVE_URLS``. If set to ``True``,
    URLs generated by ``url_for()`` that would end with ``/index.html`` will end
    with simply ``/`` instead. Defaults to ``False``.
...
* Add the ``FREEZER_RELATIVE_URLS_PRETTY`` configuration to allow generating
  relative URLs ending with just a trailing slash instead of ``/index.html``.
